...
# n8n v2.9 exact venv structure
RUN python3 -m venv /tmp/python-venv && \
    chown -R node:node /tmp/python-venv && \
    chmod -R 755 /tmp/python-venv && \
    su-exec node /tmp/python-venv/bin/python -m pip install --no-cache-dir --upgrade pip && \
    su-exec node /tmp/python-venv/bin/python -m pip install --no-cache-dir requests numpy pandas
